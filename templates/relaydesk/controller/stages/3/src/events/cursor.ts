...
export function decodeCursor(encoded: string): EventCursor {
  ...
  if (typeof cursor.tenantId !== "string" || typeof cursor.createdAt !== "string" || typeof cursor.id !== "string" || cursor.tenantId.length === 0 || cursor.createdAt.length === 0 || cursor.id.length === 0) {
