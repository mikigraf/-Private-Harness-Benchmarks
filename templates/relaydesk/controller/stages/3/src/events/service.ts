...
export class EventService {
  ...
  create(tenantId: string, type: string, payload: Record<string, unknown>, key?: string) {
    ...
    const scopedKey = key === undefined ? undefined : `${tenantId}\u0000${key}`;
    const existing = scopedKey === undefined ? undefined : this.idempotency.get(scopedKey);
    if (existing) {
    ...
    if (scopedKey !== undefined) this.idempotency.set(scopedKey, { fingerprint, event });
